...
export class Scripts {
  ...
  private buildContext(): ScriptContext {
    ...
    return {
      ...
      toKey: (type: string) => this.queue.toKey(type),
      ...
      get redisVersion() {
        return self.queue.redisVersion;
      },
      get databaseType() {
        return self.queue.databaseType;
      },
      execCommand: (
        client: RedisClient | ChainableCommander,
        commandName: string,
        args: any[],
      ) => this.execCommand(client, commandName, args),
    };
  }
  ...
  // ─── Error handling (kept on facade, extracted to script-utils in features) ─
