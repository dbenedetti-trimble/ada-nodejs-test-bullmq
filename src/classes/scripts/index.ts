...
export class Scripts {
  ...
  constructor(protected queue: ScriptQueueContext) {
    const ctx: ScriptContext = {
      ...
      toKey: this.queue.toKey.bind(this.queue),
    ...
    this.schedulerScripts = new SchedulerScripts(ctx);
    this.workerScripts = new WorkerScripts(ctx);

    this.moveToFinishedKeys = this.workerScripts.moveToFinishedKeys;
  }
  ...
  // --- Shared utilities (backward compatibility) ---

  protected getKeepJobs(
    shouldRemove: undefined | boolean | number | KeepJobs,
    workerKeepJobs: undefined | KeepJobs,
  ) {
    return getKeepJobsFn(shouldRemove, workerKeepJobs);
  }
