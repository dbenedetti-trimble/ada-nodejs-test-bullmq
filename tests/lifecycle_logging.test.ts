...
describe('Lifecycle Logging', () => {
  ...
  it('logs job:retrying at warn level when job is configured with attempts > 1 (VAL-03)', async () => {
    ...
    expect(retrying!.jobId).toBeDefined();
  ...
  it('logs job:stalled at warn level when job lock expires (VAL-04)', async () => {
    ...
    const worker1 = new Worker(queueName, async () => delay(10000), {
      connection,
      prefix,
      lockDuration: 1000,
      stalledInterval: 100,
  ...
  it('emits events in correct order across retry cycle (VAL-07)', async () => {
    ...
    await loggedQueue.add('cycleJob', {}, { attempts: 2 });
    await new Promise<void>(resolve => worker.on('completed', () => resolve()));
  ...
  it('LifecycleLogger, LifecycleLogEntry, and LifecycleEvent are importable from bullmq src (VAL-08)', () => {
  ...
  it('shouldLog returns false for events not in logEvents — verifiable via access on instance', async () => {
